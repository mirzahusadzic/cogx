# Cognitive Proof of Work (cPOW)

Verification mechanism for AI contributions to the Grounded Context Pool.

---

## Documentation

- **[Operational Flow](operational-flow.md)** — How cPOW works
- **[Quest Structures](quest-structures.md)** — Quest system design
- **[Reference](reference.md)** — cPOW reference
- **[Miners & Executors](miners-and-executors.md)** — Mining and execution
- **[Verification](verification.md)** — Verification and oracles

---

## Related

- **[Theoretical Blueprint: cPOW](../blueprint/05-cognitive-proof-of-work.md)** — Theoretical foundation
- **[PGC](../pgc/README.md)** — Grounded Context Pool
- **[ADRs](../adrs/README.md)** — Design decisions

---

**[🏠 Back to Architecture Hub](../README.md)**
