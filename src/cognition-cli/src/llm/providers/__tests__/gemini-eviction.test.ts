...
vi.mock('@google/adk', () => {
  ...
  const mockSessionService = {
    getSession: vi.fn().mockResolvedValue({ sessionId: 'test-session', events: [] }),
    createSession: vi.fn().mockResolvedValue({ sessionId: 'test-session', events: [] }),
    appendEvent: vi.fn().mockResolvedValue(undefined),
  };
  return {
    LlmAgent: vi.fn().mockImplementation(() => ({})),
    Runner: vi.fn().mockImplementation(() => mockRunner),
    InMemorySessionService: vi.fn().mockImplementation(() => mockSessionService),
    setLogLevel: vi.fn(),
    LogLevel: { ERROR: 0 },
    StreamingMode: { SSE: 'sse', BIDI: 'bidi' },
    GOOGLE_SEARCH: {},
    AgentTool: vi.fn(),
    FunctionTool: vi.fn().mockImplementation(({ name, description, parameters, execute }) => ({
      name,
      description,
      parameters,
      execute,
...
describe('Gemini Eviction Strategy', () => {
  ...
  describe('Log Eviction (pruneTaskLogs)', () => {
    it('should surgically evict logs with matching task tags', async () => {
      ...
      const resultSummary = 'Successfully implemented the feature with 5 tests.';
          ...
          content: { 
            parts: [
              { 
                functionResponse: { 
                  name: 'read_file', 
                  response: { result: `file content here\n\n<!-- sigma-task: ${taskId} -->` } 
                } 
              }
            ] 
          }
      ...
      expect(part.functionResponse.response.result).toContain(`Task ${taskId} completed`);
  ...
  describe('SigmaTaskUpdate Validation Gate', () => {
    it('should throw error when task completed without summary', async () => {
      ...
      const sigmaTool = tools.find((t: any) => t.name === 'SigmaTaskUpdate');
      const input = {
        todos: [{ id: 'task-1', content: 'Do', status: 'completed', activeForm: 'Doing' }],
      };
    ...
    it('should call onTaskCompleted when task is marked completed', async () => {
      ...
      const tools = getCognitionTools(undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, { onTaskCompleted, anchorId: 'test', provider: 'gemini' });
      const sigmaTool = tools.find((t: any) => t.name === 'SigmaTaskUpdate');
      const resultSummary = 'This is a long enough summary for the task completion.';
      await sigmaTool.execute({
        todos: [{ id: 'task-1', content: 'Do', status: 'completed', activeForm: 'Doing', result_summary: resultSummary }],
      });
      expect(onTaskCompleted).toHaveBeenCalledWith('task-1', resultSummary, undefined);
    });
  });
  ...
  describe('Active Context Loading', () => {
    it('should load active_context.md into the instruction when it exists', async () => {
      ...
      const activeContextContent = 'Existing critical finding: API key is in config.ts';
      ...
      for await (const _ of generator) {
        if (vi.mocked(LlmAgent).mock.calls.some(c => c[0].name === 'cognition_agent')) break;
      }

      const cognitionAgentCall = vi.mocked(LlmAgent).mock.calls.find(c => c[0].name === 'cognition_agent');
      expect(cognitionAgentCall).toBeDefined();
      expect(cognitionAgentCall![0].instruction).toContain('Long Term Working Memory');
      expect(cognitionAgentCall![0].instruction).toContain(activeContextContent);
    });
  });
});
