...
const sessionStatePath = path.join(__dirname, 'dist', 'sigma', 'session-state.js');
const {
  createSessionState,
  updateSessionState,
  saveSessionState,
...
saveSessionState(state1, __dirname.replace('/src/cognition-cli', '/src/cogx/src/cognition-cli/.test-sigma/..'));
...
saveSessionState(state2, __dirname.replace('/src/cognition-cli', '/src/cogx/src/cognition-cli/.test-sigma/..'));

let state3 = updateSessionState(state2, 'sdk-session-3', 'compression', 131024);
saveSessionState(state3, __dirname.replace('/src/cognition-cli', '/src/cogx/src/cognition-cli/.test-sigma/..'));

let state4 = updateSessionState(state3, 'sdk-session-4', 'compression', 134385);
saveSessionState(state4, __dirname.replace('/src/cognition-cli', '/src/cogx/src/cognition-cli/.test-sigma/..'));

console.log(`  ✓ Added 3 compressions`);
console.log(`  Compression history length: ${state4.compression_history.length}`);
console.log(`  Current session: ${state4.current_session}`);

// Step 3: Verify state has 4 entries
console.log('\nStep 3: Verify state file');
const beforeRestart = loadSessionState(anchorId, __dirname.replace('/src/cognition-cli', '/src/cogx/src/cognition-cli/.test-sigma/..'));
console.log(`  Compression history: ${beforeRestart.compression_history.length} entries`);
console.log(`  Sessions: ${beforeRestart.compression_history.map(h => h.reason).join(' → ')}`);

// Step 4: Simulate TUI restart with new SDK session
console.log('\nStep 4: Simulate TUI restart (THE FIX TEST)');
console.log('  This simulates what happens when TUI restarts with same anchor_id');
console.log('  OLD BUG: Would call create() and reset state to 1 entry');
console.log('  NEW FIX: Should call update() with reason="restart" and preserve history');

// The fix: Use updateSessionState with 'restart' reason instead of createSessionState
const afterRestart = updateSessionState(beforeRestart, 'sdk-session-5', 'restart');
saveSessionState(afterRestart, __dirname.replace('/src/cognition-cli', '/src/cogx/src/cognition-cli/.test-sigma/..'));

console.log(`  ✓ Updated with reason='restart'`);
console.log(`  Compression history length: ${afterRestart.compression_history.length}`);
console.log(`  Current session: ${afterRestart.current_session}`);

// Step 5: Verify state preserved
console.log('\nStep 5: Verify state was preserved');
const final = loadSessionState(anchorId, __dirname.replace('/src/cognition-cli', '/src/cogx/src/cognition-cli/.test-sigma/..'));
const historyPreserved = final.compression_history.length === 5;
const hasRestartEntry = final.compression_history.some(h => h.reason === 'restart');

console.log(`  Compression history: ${final.compression_history.length} entries`);
console.log(`  Sessions: ${final.compression_history.map(h => h.reason).join(' → ')}`);
...
if (historyPreserved && hasRestartEntry) {
  ...
  console.log(`   Expected: 5 entries, Got: ${final.compression_history.length}`);
