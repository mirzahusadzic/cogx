...
41. **Intelligent Context Compression (Configurable Threshold):** User-configurable threshold via `--session-tokens` CLI parameter (default: 150K tokens), importance-based filtering using formula `novelty × 5 + max(alignment_O1..O7) × 0.5` with high-alignment preservation (≥6) and low-value chat discarding, generating 7-dimensional intelligent recaps via lattice algebra
...
44. **Periodic Overlay Persistence:** Automatic flush every 5 turns preventing data loss in short sessions, cleanup flush on TUI exit/unmount guaranteeing data preservation, overlays remaining in memory across SDK session boundaries, memory available before compression trigger (configurable threshold)
...
46. **Interactive TUI with Real-Time Lattice Visualization:** Production-ready terminal interface with live status bar featuring a visual proportional bar for cached vs active tokens, semantic compression marker (Σ), and flicker-free session-wide cumulative tracking. Includes live overlay status (O1-O7), lattice statistics (nodes/edges/shifts), toggle info panel, and persistent scroll history with mouse support.
...
- **Cognitive Prosthetics** (`docs/research/cognitive-prosthetics.md`) - Memory healing through verifiable architecture
- **Neural-Memory Protocol** (`docs/research/neural-memory-protocol.md`) - BCI abstraction layer specification
